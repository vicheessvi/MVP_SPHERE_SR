...
- Текущая feature: `specs/019-close-insecure-ports/`.
...
- Write-задача `disable_insecure_management_services` ограничена exact Huawei TE40, plan schema v3 и явным opt-in. Разрешены только `enabletelnet=0` и `enable_http=1`; успех требует post-read, TCP/23 closed, TCP/80 closed и TCP/443 open. Тестовый firmware сохраняет TCP/80 открытым, поэтому live acceptance остаётся незавершённым и такой исход MUST быть `configuration_verification_failed`, а не успехом.
