...
- Write-задача `disable_insecure_management_services` ограничена exact Huawei TE40, plan schema v3 и явным opt-in. Разрешены только `enabletelnet=0` и `enable_http=1`; по решению пользователя успех определяется повторным чтением этих значений из меню кодека. TCP listener не входит в критерий результата задачи.
