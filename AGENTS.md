...
- Текущая feature: `specs/022-huawei-te20-polling/`.
...
- Для exact Huawei TE20 разрешён отдельный transport `huawei_te20_web_cgi_v1` в `mvp_runtime/adapters/huawei_te40.py`: HTTPS/443, точный TLS 1.1 только для этой модели, подтверждённая legacy pre-auth схема, проверка exact TE20 сразу после входа и фиксированный action allowlist. Для точных Huawei TE30/TE40/TE50/TE60 разрешён общий guarded transport `huawei_te_web_cgi_v1` с проверкой модели до credentials и после входа. Все пять моделей подтверждены контролируемыми опросами реальных устройств. Другие Huawei модели остаются `protocol_required`.
- Write-задача `disable_insecure_management_services` ограничена exact Huawei TE20/TE30/TE40/TE50/TE60, plan schema v3 и явным opt-in. Безопасные цели зависят от exact-модели: для TE20 разрешены только `enabletelnet=0` и `enable_http=0`, для TE30/TE40/TE50/TE60 — `enabletelnet=0` и `enable_http=1`; успех определяется повторным чтением модельных значений из меню кодека. TCP listener не входит в критерий результата. Реальная write-проверка подтверждена для TE20 и TE40; TE30/TE50/TE60 сохраняют provisional-статус до контролируемой проверки пользователем, а model/identity/firmware/schema drift блокирует запись.
