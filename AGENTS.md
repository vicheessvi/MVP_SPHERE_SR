...
- Для exact Huawei TE20 разрешён отдельный read-only transport `huawei_te20_web_cgi_v1` в `mvp_runtime/adapters/huawei_te40.py`: HTTPS/443, точный TLS 1.1 только для этой модели, подтверждённая legacy pre-auth схема, проверка exact TE20 сразу после входа и фиксированный action allowlist. Для точных Huawei TE30/TE40/TE50/TE60 разрешён общий guarded transport `huawei_te_web_cgi_v1` с проверкой модели до credentials и после входа. Все пять моделей подтверждены контролируемыми опросами реальных устройств. Другие Huawei модели остаются `protocol_required`.
- Write-задача `disable_insecure_management_services` ограничена exact Huawei TE30/TE40/TE50/TE60, plan schema v3 и явным opt-in. Разрешены только `enabletelnet=0` и `enable_http=1`; по решению пользователя успех определяется повторным чтением этих значений из меню кодека. TCP listener не входит в критерий результата задачи. Реальная write-проверка подтверждена для TE40; TE30/TE50/TE60 сохраняют provisional-статус до контролируемой проверки пользователем, а firmware/schema drift блокирует запись.
