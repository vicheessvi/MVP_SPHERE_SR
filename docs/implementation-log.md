...
Commit, push и deploy не выполнялись.
## 2026-09-01 — Миграция production runtime на Python

Выполнена Full SpecKit feature `014-python-runtime-migration`. Автоматический режим перенесён с Node.js/PowerShell/CMD на установленный Python 3.11+ без сторонних пакетов. `START_MVP_SPHERE_SR.py` проверяет целостность локального пакета, поднимает случайный loopback-порт и открывает одноразовую HttpOnly/CSRF-сессию; прямой `index.html` остаётся ручным режимом.
...
После пользовательского подтверждения реального файла правило срочно скорректировано до `extron-reboot-v2`: каждый файл вычисляет последний запуск как `Device Status.Date − Device Status.Uptime`; один файл достаточен для события, одинаковые времена запуска объединяются с допуском пять секунд, новое время считается следующей перезагрузкой. Противоречивые Date/uptime, неоднозначные timestamps, несопоставленные и неполные результаты исключаются. При недоступном Time Zone результат показывается как локальное время устройства без преобразования. `uptimeObservedAt`, top-level `capturedAt` и timestamp файла остаются fallback provenance. Локация и адрес подписаны как данные актуальной SR.
...
## 2026-09-05 — Feature 017: адресный опрос и фильтр домена

План автоматического опроса получил два взаимоисключающих способа выбора. Режим «По IP-адресу» находит ровно одно устройство по нормализованному полному IPv4 только в актуальной SR и показывает под полем карточку с типом, производителем, моделью, IP/MAC, доменом, локацией, адресом, инвентарным и серийным номерами и статусом поддержки. Пустой, некорректный, отсутствующий, исторический или дублирующийся IP не формирует сетевой план; похожие и соседние адреса не используются.

Режим «По фильтрам» сохраняет существующую логику пересечения и дополнен первым уровнем `Домен → Тип оборудования → Производитель → Модель`. Доступны один, несколько, все домены и «Не указано»; при смене родительского значения недоступные дочерние варианты очищаются. Отсутствие явного `domains` в прежнем вызове трактуется как «Все», поэтому старое массовое поведение сохраняется.

Plan schema v2, Python server/job/adapters, credentials, schedule, interval, TLS-разрешение, exact-IP allowlist, ACK и локальное сохранение JSON не изменялись. Raw-строка IP-поиска не записывается в plan. Справочник и архитектурная документация синхронизированы; решение описано в ADR-0017 и Full SpecKit feature `017-polling-target-domain`. Все тестовые адреса используют только documentation ranges, реальные SR и результаты опросов не добавлялись. Commit и push не выполнялись.

## 2026-09-05 — Feature 018: общий guarded-опрос Huawei TE30/TE40/TE50/TE60

На основании официального Huawei HTTP API reference V600R019C00 для общего семейства TE30/TE40/TE50/TE60/TX50 выдвинутая пользователем гипотеза оформлена как один transport для точных моделей TE30, TE40, TE50 и TE60. TX50 намеренно не включён в scope. Каталог, frontend capability и runtime registry используют один canonical transport; отдельные копии алгоритма по моделям не создавались.

До отправки username/password через web интерфейс adapter проверяет прежние login bundle markers и отдельный токен ожидаемой модели в `WEB_GetLoginInfo.szTermType`. После входа тот же planned model повторно проверяется в `WEB_GetVersionInfoAPI.model`, а identity требует serial или валидный MAC. Несовпадение до входа завершает запрос без credential action; неизвестный bundle, firmware/schema drift и другая модель отклоняются fail-closed. Exact-IP allowlist, HTTPS/443, one-run TLS exception, шесть read-only actions, bounds, redaction, sequential job/ACK и локальный JSON не изменены.

После внедрения пользователь подтвердил корректное завершение контролируемых опросов реальных TE30, TE50 и TE60 по той же последовательности, которая ранее была проверена на TE40. Гипотеза общего алгоритма подтверждена для всех четырёх моделей и проверенных установленных прошивок; provisional-статус снят. Runtime fingerprinting и fail-closed проверки модели сохраняются как защита от подмены цели и будущего firmware/schema drift. Решение описано в ADR-0018 и Full SpecKit feature `018-huawei-te-family`. Реальные IP, MAC, credentials, raw responses и результаты в репозиторий не добавлялись.

## 2026-09-06 — Feature 019: opt-in закрытие HTTP/Telnet на Huawei TE40

В План автоматического опроса добавлена выключенная по умолчанию задача «Закрыть порты 80, 23 (http, telnet)», отдельное подтверждение и счётчики поддерживаемых/пропускаемых целей. Plan повышен до schema v3 с явным `managementTasks`, чтобы прежний runtime не мог молча игнорировать write-intent. Capability ограничена exact TE40; TE30/TE50/TE60 продолжают обычный опрос и получают `skipped_unsupported` для изменения.

По уточнению пользователя источник целей write-задачи отделён от основной SR. В разделе 4 загружается XLSX «Список устройств для закрытия портов 80 и 23 (http, telnet)» той же структуры: при выбранной задаче каскад и точный IP используют только этот список. Он атомарно отклоняется при отсутствующем/некорректном или повторяющемся IP, существует только в памяти и не добавляет записи в inventory, locations или аналитику. План сохраняет только безопасную runtime-проекцию целей, тип источника и SHA-256 файла.

Firmware evidence подтвердил `WEB_GetCfgParamAPI`, `WEB_SaveCfgParamAPI`, `enabletelnet=0` и `enable_http=1`. Адаптер выполняет действие только после двойной проверки модели и identity, отправляет минимальный payload, допускает пустой `data` успешного save-envelope и повторно читает значения меню. Результат содержит только безопасные состояния и коды; credentials/cookie/CSRF/raw response не сохраняются.

Контролируемый тест реального TE40 подтвердил значения security UI; TCP/80 при этом оставался доступен. По последующему уточнению пользователя задача должна менять и подтверждать именно значения меню кодека, чего достаточно для успешного отчёта; TCP listener исключён из критерия и больше не вызывает `configuration_verification_failed`. Ошибка сохраняется, если post-read не подтверждает целевые значения. Полная локальная проверка завершена: 46 Python-тестов и 154 JavaScript-теста, compileall, JS syntax, reference validation, diff check и secret scan прошли успешно. Решение описано в ADR-0019 и Full SpecKit feature `019-close-insecure-ports`. Реальные IP, credentials и device responses в репозиторий не добавлялись. Commit и push выполняются только по отдельному указанию пользователя.

## 2026-09-07 — Feature 020: закрытие HTTP/Telnet для Huawei TE30/TE40/TE50/TE60

По указанию пользователя существующая opt-in write-задача расширена с exact TE40 на закрытый список TE30, TE40, TE50 и TE60. Все четыре модели используют один общий `huawei_te_web_cgi_v1` pipeline; отдельные модельные реализации не создавались. Источник целей, фильтры, точный IP, plan schema v3, подтверждение пользователя, HTTPS/443 и сеансовые credentials остались без изменения.

Перед записью сохраняются exact model checks до credentials и после входа, identity, наличие `WEB_GetCfgParamAPI`/`WEB_SaveCfgParamAPI` и обоих config markers, а также проверка схемы текущих значений. Единственный допустимый payload остаётся `enabletelnet=0`, `enable_http=1`; успех определяется post-read значений меню. Synthetic tests для каждой модели подтверждают один минимальный save, а marker/schema drift — ноль save. TE20, TX50 и неизвестные Huawei не получают capability.

Полная проверка: 47 Python-тестов, 154 JavaScript-теста, compileall, JS syntax и reference validation прошли. Secret/artifact/IP scan использовал только документационные адреса в тестах и не обнаружил добавленных credentials, приватных IP или runtime artifacts; `git diff --check` прошёл. Реальная write-проверка остаётся подтверждённой для TE40; TE30/TE50/TE60 отмечены provisional до контролируемого запуска пользователя. Решение описано в ADR-0020 и Full SpecKit feature `020-huawei-te-family-port-closure`. Commit и push не выполнялись.

## 2026-09-08 — Feature 021: актуальная схема выгрузки SR

Принят новый канонический контракт выгрузки SR из 42 уникальных столбцов: от `SmartRoom ID локации` до `802.1x`. Переименованные прежние заголовки сохранены как входные псевдонимы с приоритетом нового имени; обязательность проверяется по смысловым полям, поэтому legacy-файлы продолжают загружаться, а ошибка сообщает актуальное название отсутствующей колонки. Основная SR и отдельный список целей закрытия портов используют один parser.

Нормализованная локальная проекция дополнена статусами локации и оборудования, комментариями учёта, заявками, контактами, подрядчиком, гарантией, питанием, интерфейсом, IP-протоколом и `802.1x`; raw XLSX row остаётся неизменённой. `SmartRoom ID локации` и `SmartRoom ID оборудования` стали приоритетными устойчивыми идентификаторами, `ID устройства` учитывается до изменяемых сетевых реквизитов. Повторная выгрузка с теми же ID обновляет существующие сущности и историю IP без дубликатов.

Интерфейс, фильтры, карточки и Справочник используют `Наименование локации`, `Класс оборудования`, `Тип`, `IP адрес` и `MAC адрес`. Полная проверка завершена: 47 Python-тестов и 157 JavaScript-тестов, compileall, JS syntax, reference validation и `git diff --check` прошли. Secret/artifact/private-IP scan не обнаружил добавленных credentials, runtime artifacts или инфраструктурных адресов. Решение описано в ADR-0021 и Full SpecKit feature `021-sr-export-schema`. Commit и push не выполнялись.

## 2026-09-08 — Feature 022: автоматический опрос Huawei TE20

Controlled live evidence подтвердил отдельные особенности TE20: HTTPS/443 принимает точный TLS 1.1, `WEB_GetLoginInfo` до входа возвращает legacy numeric schema без текстовой модели, а после входа устройство объявляет exact TE20, устойчивую identity и те же шесть read-only ресурсов, что общий TE30/TE40/TE50/TE60 pipeline. Файл credentials прошёл bounded XLSX parser; использована одна пара, её значения, cookie и CSRF не выводились и не сохранялись.

Добавлен отдельный transport `huawei_te20_web_cgi_v1` и exact manifest `vcs/huawei/te20-legacy`. TLS 1.1 min/max и legacy cipher profile применяются только к TE20. До credentials обязательны exact plan target, login markers и подтверждённая numeric pre-auth схема; сразу после входа обязательны exact TE20 и serial либо MAC. Затем формируется прежний совместимый JSON с `Device Info`, `Firmware`, `Device Status`, `LAN Settings` и `Capabilities`. HTTP fallback отсутствует.

TE20 намеренно не получил write-capability закрытия HTTP/Telnet; она остаётся только у TE30/TE40/TE50/TE60. Production routing успешно прошёл controlled live poll через общий каталог и новый adapter key. Полная проверка: 49 Python-тестов и 157 JavaScript-тестов, compileall, JS syntax, reference validation, diff и security scan. Реальные credentials, IP, MAC, serial и результаты опроса в репозиторий не добавлялись. Решение описано в ADR-0022 и Full SpecKit feature `022-huawei-te20-polling`. Commit и push не выполнялись.

## 2026-09-08 — Feature 023: закрытие HTTP/Telnet для Huawei TE20

После подтверждения обычного опроса пользователь потребовал проверить write-задачу на exact TE20. Первый контролируемый сеанс разрешал вход и чтение, но блокировал save до отправки: bundle markers и точная схема обоих параметров подтвердились, а устройство уже сообщало безопасное состояние. Затем выполнена идемпотентная контрольная запись только `enabletelnet=0` и `enable_http=1`; один save был принят, а второе чтение подтвердило оба безопасных значения. Небезопасные службы для теста не включались.

TE20 добавлен в существующую capability `disable_insecure_management_services`, сохраняя собственный `huawei_te20_web_cgi_v1`, exact TLS 1.1 и legacy pre-auth gate. Python и JavaScript resolvers теперь возвращают transport exact device manifest, поэтому действие не переносит TE20 в общий TE30–TE60 transport. Plan schema v3, отдельный XLSX-список целей, фильтры, точный IP, confirmation, минимальный payload и post-read result contract не изменены. Другие Huawei-модели остаются неподдерживаемыми.
