...
Firmware evidence подтвердил `WEB_GetCfgParamAPI`, `WEB_SaveCfgParamAPI`, `enabletelnet=0` и `enable_http=1`. Адаптер выполняет действие только после двойной проверки модели и identity, отправляет минимальный payload, допускает пустой `data` успешного save-envelope, повторно читает значения и проверяет TCP/23 closed, TCP/80 closed, TCP/443 open. Результат содержит только безопасные состояния и коды; credentials/cookie/CSRF/raw response не сохраняются.

Контролируемый тест реального TE40 подтвердил закрытый TCP/23, доступный TCP/443 и значения security UI, но TCP/80 остался открыт и вернул HTTP 200. Поэтому live acceptance не закрыт: алгоритм обязан вернуть `configuration_verification_failed`, а не ложный успех. Отдельного listener config ID в bundle не найдено; дальнейшая запись или перезапуск требуют нового подтверждения. Полная локальная проверка завершена: 46 Python-тестов и 154 JavaScript-теста, compileall, JS syntax, reference validation, diff check и secret scan прошли успешно. Решение описано в ADR-0019 и Full SpecKit feature `019-close-insecure-ports`. Реальные IP, credentials и device responses в репозиторий не добавлялись. Commit и push не выполнялись.
