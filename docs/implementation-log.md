...
TE40 остаётся единственной моделью с выполненной ранее реальной проверкой. TE30, TE50 и TE60 включены как provisional compatibility behind runtime fingerprinting: окончательное подтверждение каждой модели требует контролируемого адресного опроса и анализа redacted JSON. Решение описано в ADR-0018 и Full SpecKit feature `018-huawei-te-family`. Реальные IP, credentials, raw responses и результаты в репозиторий не добавлялись. Commit и push не выполнялись.
