...
- `mvp_runtime/adapters/huawei_te40.py` реализует один guarded Huawei TE30/TE40/TE50/TE60 HTTPS web-CGI contract с проверкой ожидаемой модели до credentials и после входа и с фиксированным read-only allowlist.
...
План автоматического опроса имеет два взаимоисключающих способа выбора. Адресный режим нормализует введённый IPv4, ищет ровно одно совпадение среди устройств актуальной SR и показывает локальную карточку без сетевого обращения; некорректный, отсутствующий, исторический или неоднозначный IP даёт пустой набор. Массовый режим использует прежнее пересечение, дополненное первым уровнем `Домен → Тип оборудования → Производитель → Модель`; один, несколько, все и пустые домены обрабатываются теми же правилами каскада. Строка поиска IP не сохраняется в плане: неизменный plan v2 передаёт runtime только разрешённые `deviceIds` и `devices[]`.
