...
Добавить между «Главным экраном» и «Оборудованием» отдельный модуль, который из полной хронологии однозначно сопоставленных результатов опроса строит доказуемую нижнюю границу перезагрузок. Правило `reboot-min-v1` использует строго поддерживаемый uptime и время наблюдения, вычисляет интервал последнего запуска и подтверждает минимум одну перезагрузку только когда этот интервал целиком позже предыдущего наблюдения. Производные события остаются в памяти сеанса; единый отфильтрованный набор питает KPI, максимумы, доступные HTML/CSS-графики и таблицу.
...
- **III. Детерминизм**: версия правила `reboot-min-v1`; сортировка и обработка ties детерминированы.
