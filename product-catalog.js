(function (root, factory) {
  ...
  const EQUIPMENT_CATEGORY_CATALOG = deepFreeze([
    { id: "vcs", route: "vcs", title: "Терминалы ВКС", order: 21, srField: "Тип", srValue: "Video Conference", pollingProtocol: "protocol_required" },
    { id: "controller", route: "controllers", title: "Контроллеры", order: 22, srField: "Класс оборудования", srValue: "controller", pollingProtocol: "protocol_required" },
    { id: "panel", route: "panels", title: "Панели управления", order: 23, srField: "Тип", srValue: "Панель управления", pollingProtocol: "protocol_required" },
    { id: "switch", route: "switches", title: "Коммутаторы", order: 24, srField: "Тип", srValue: "Коммутатор", pollingProtocol: "protocol_required" },
    { id: "matrix_switch", route: "matrix-switches", title: "Матричные коммутаторы", order: 25, srField: "Тип", srValue: "Матричный коммутатор", pollingProtocol: "protocol_required" },
    { id: "scaler", route: "scalers", title: "Скалеры", order: 26, srField: "Тип", srValue: "Скалер", pollingProtocol: "protocol_required" },
    { id: "audio_processor", route: "audio-processors", title: "Аудио процессоры", order: 27, srField: "Тип", srValue: "Аудио процессор", pollingProtocol: "protocol_required" }
  ]);

  const HUAWEI_TE_MODELS = deepFreeze(["te20", "te30", "te40", "te50", "te60"]);
  ...
  const ANALYZED_PARAMETER_RULES = deepFreeze([
    ...
    ...HUAWEI_TE_MODELS.flatMap((modelNormalized) => [
      { id: `huawei-${modelNormalized}-model-v1`, category: "vcs", manufacturerNormalized: "huawei", modelNormalized, path: "$.webBlocks.Device Info.Model", label: "Модель терминала ВКС", rationale: `Подтверждает модель Huawei ${modelNormalized.toUpperCase()} в результате ${modelNormalized === "te20" ? "отдельного legacy" : "общего"} алгоритма опроса.`, version: 1 },
      { id: `huawei-${modelNormalized}-firmware-v1`, category: "vcs", manufacturerNormalized: "huawei", modelNormalized, path: "$.webBlocks.Firmware.Version", label: "Версия прошивки", rationale: `Показывает установленную версию прошивки терминала ВКС Huawei ${modelNormalized.toUpperCase()}.`, version: 1 }
    ])
  ]);
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-huawei-te20-web-polling", title: "Автоматический опрос Huawei TE20", summary: "TE20 использует отдельный подтверждённый web-CGI transport с точным legacy TLS-профилем.", details: "Алгоритм выбирается только для точного сочетания «Терминалы ВКС / Huawei / TE20». Устройство принимает HTTPS с точным TLS 1.1; ослабленный профиль применяется только к TE20 и только при явном разрешении самоподписанного сертификата для текущего запуска. До пароля проверяется подтверждённая legacy login-схема, после входа устройство обязательно объявляет модель TE20 и подтверждает serial либо MAC. Затем читаются те же шесть ограниченных ресурсов устройства, прошивки, времени, сети и возможностей. При отдельном явном выборе задачи TE20 также поддерживает подтверждённое закрытие HTTP/Telnet через минимальную запись двух безопасных значений и обязательное повторное чтение. Учётные данные, cookie и CSRF остаются только в памяти задания.", keywords: ["huawei", "te20", "терминал вкс", "автоматический опрос", "https", "tls 1.1", "mac", "прошивка", "http", "telnet"] },
    { id: "logic-huawei-te-web-polling", title: "Автоматический опрос Huawei TE30 / TE40 / TE50 / TE60", summary: "Четыре модели используют один подтверждённый защищённый алгоритм входа и чтения параметров общего web-CGI семейства.", details: "Алгоритм выбирается только при точном сочетании категории «Терминалы ВКС», производителя Huawei и модели TE30, TE40, TE50 или TE60. До отправки пароля устройство должно объявить ожидаемую модель; после входа модель повторно подтверждается по сведениям версии вместе с серийным номером или MAC. Используется HTTPS по выбранному IP и только разрешённый набор сведений об устройстве, прошивке, локальном времени, сети и возможностях. Совместимость общего алгоритма подтверждена контролируемыми опросами реальных TE30, TE40, TE50 и TE60. Учётные данные, cookie и CSRF-токен существуют только в памяти задания и не записываются в итоговый JSON.", keywords: ["huawei", "te30", "te40", "te50", "te60", "терминал вкс", "автоматический опрос", "https", "mac", "прошивка"] },
    { id: "logic-sr-xlsx", title: "Выгрузка SR", summary: "XLSX с актуальным перечнем локаций и оборудования формирует варианты Домена, Класса оборудования, Производителя и Модели.", details: "Читается первый непустой лист. Актуальная схема использует заголовки «SmartRoom ID локации», «Наименование локации», «Адрес локации», «Признак VIP», «SmartRoom ID оборудования», «Класс оборудования», «Наименование оборудования», «Тип», «IP адрес» и «MAC адрес». Все 42 объявленных столбца принимаются и сохраняются в исходной строке; поля, нужные для перечня и аналитики, также нормализуются. Прежние заголовки поддерживаются как совместимые алиасы. Обязательные смысловые поля проверяются до изменения сеансового состояния. Столбец «Домен» используется как первый уровень фильтра Плана автоматического опроса; пустое значение доступно как «Не указано».", keywords: ["sr", "xlsx", "первый лист", "импорт", "домен", "локация", "класс оборудования", "ip адрес", "mac адрес"] },
    ...
    { id: "logic-automatic-polling-plan", title: "План автоматического опроса", summary: "План выбирает одно устройство по точному IP либо строит набор каскадом Домен → Класс оборудования → Производитель → Модель.", details: "Обычный опрос использует актуальную SR. Для выключенной по умолчанию задачи «Закрыть порты 80, 23 (http, telnet)» загружается отдельный одноимённый XLSX той же структуры: фильтры и поиск по IP работают только по нему, а его строки не добавляются в SR, оборудование или аналитику. Некорректный либо повторяющийся IP блокирует весь отдельный список. Изменение выполняется после успешного опроса для точных Huawei TE20, TE30, TE40, TE50 и TE60 и только после повторной проверки модели, идентичности и firmware-контракта; остальные цели безопасно пропускаются. Для TE20 и семейства TE30–TE60 применяются отдельные подтверждённые значения HTTP; успех фиксируется только после повторного чтения отключённого состояния HTTP и Telnet в меню кодека. TCP listener не входит в критерий отчёта. Перед запуском требуется подтверждение, а безопасный результат сохраняется в JSON устройства без учётных данных.", keywords: ["план", "список устройств", "xlsx", "ip-адрес", "домен", "класс оборудования", "производитель", "модель", "закрыть порты", "http", "telnet", "te20", "te30", "te40", "te50", "te60"] },
  ...
  const equipmentModules = EQUIPMENT_CATEGORY_CATALOG.map((category) => ({
    ...
    details: `Использует общий перечень, фильтры, карточку, историю опросов и аналитику оборудования. Категория определяется точным сравнением поля SR «${category.srField}» со значением «${category.srValue}» после безопасной нормализации пробелов и регистра. Каждая различимая строка актуальной SR остаётся в перечне даже без результата опроса.${["controller", "panel"].includes(category.id) ? " Для устройств Extron доступен отдельный локальный contract-based web-опрос; ручная загрузка JSON и папок сохраняется." : category.id === "vcs" ? " Для Huawei TE20 доступен отдельный legacy TLS/web-CGI опрос, а TE30, TE40, TE50 и TE60 используют общий guarded web-CGI опрос; остальные модели требуют подтверждения собственного протокола. Ручная загрузка JSON и папок сохраняется." : " Подтверждённый протокол автоматического опроса не предполагается без отдельной спецификации."}`,
  ...
  const MODULE_CATALOG = deepFreeze([
    ...
    ...equipmentModules,
    {
      ...
      details: "Операционный экран предлагает два способа выбора: одно актуальное устройство по точному IP с карточкой данных SR либо массовый каскад Домен, Класс оборудования, Производитель, Модель. Он показывает выбранные, поддерживаемые и неподдерживаемые устройства. XLSX учётных данных содержит только «Логин» и «Пароль»; секреты остаются в памяти текущего задания. Для Extron контроллеров и панелей применяется отдельный HTTPS-алгоритм; Huawei TE20 использует отдельный legacy TLS/web-CGI алгоритм, а TE30, TE40, TE50 и TE60 — общий guarded web-CGI алгоритм. Задача закрытия HTTP/Telnet доступна для всех пяти точных Huawei TE-моделей и сохраняет их отдельные transport-профили. Cookie и токены существуют только в памяти. После выбора общей папки кнопка формирует план и запускает локальную Python-среду; внутри папки запуска YYYY-MM-DD_HH-mm-ss каждый готовый JSON сохраняется до начала интервала. Для автоматического опроса проект запускается двойным щелчком по START_MVP_SPHERE_SR.py. Требуется установленный Python 3.11 или новее; Node.js, PowerShell, CMD, pip и права администратора не нужны. Прямой index.html сохраняет ручной импорт и аналитику без сетевого опроса.",
      keywords: ["импорт", "xlsx", "json", "общая папка", "учётные данные", "автоматический опрос", "ip-адрес", "домен", "ручная загрузка"]
    },
    {
  ...
  const STATUS_DESCRIPTORS = deepFreeze([
    ...
    { id: "status-auth", group: "pollStatuses", code: "authorization_error", summary: "Для любого устройства Extron получено точное значение error = No credentials were accepted либо для поддерживаемого Extron/Huawei TE20/TE30/TE40/TE50/TE60 подтверждён отказ на стадии авторизации.", keywords: ["authorization_error", "no credentials were accepted", "логин", "пароль", "extron", "huawei", "te20", "te30", "te40", "te50", "te60"] },
