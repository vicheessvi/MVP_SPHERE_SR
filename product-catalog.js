(function (root, factory) {
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-huawei-te20-web-polling", title: "Автоматический опрос Huawei TE20", summary: "TE20 использует отдельный подтверждённый read-only web-CGI transport с точным legacy TLS-профилем.", details: "Алгоритм выбирается только для точного сочетания «Терминалы ВКС / Huawei / TE20». Устройство принимает HTTPS с точным TLS 1.1; ослабленный профиль применяется только к TE20 и только при явном разрешении самоподписанного сертификата для текущего запуска. До пароля проверяется подтверждённая legacy login-схема, после входа устройство обязательно объявляет модель TE20 и подтверждает serial либо MAC. Затем читаются те же шесть ограниченных ресурсов устройства, прошивки, времени, сети и возможностей. Закрытие HTTP/Telnet для TE20 не поддерживается. Учётные данные, cookie и CSRF остаются только в памяти задания.", keywords: ["huawei", "te20", "терминал вкс", "автоматический опрос", "https", "tls 1.1", "mac", "прошивка"] },
    ...
    { id: "logic-automatic-polling-plan", title: "План автоматического опроса", summary: "План выбирает одно устройство по точному IP либо строит набор каскадом Домен → Класс оборудования → Производитель → Модель.", details: "Обычный опрос использует актуальную SR. Для выключенной по умолчанию задачи «Закрыть порты 80, 23 (http, telnet)» загружается отдельный одноимённый XLSX той же структуры: фильтры и поиск по IP работают только по нему, а его строки не добавляются в SR, оборудование или аналитику. Некорректный либо повторяющийся IP блокирует весь отдельный список. Изменение выполняется после успешного опроса для точных Huawei TE30, TE40, TE50 и TE60 и только после повторной проверки модели, идентичности и firmware-контракта; остальные цели безопасно пропускаются. Успех фиксируется после повторного чтения в меню кодека значений `enabletelnet=0` и `enable_http=1`; TCP listener не входит в критерий отчёта. Перед запуском требуется подтверждение, а безопасный результат сохраняется в JSON устройства без учётных данных.", keywords: ["план", "список устройств", "xlsx", "ip-адрес", "домен", "класс оборудования", "производитель", "модель", "закрыть порты", "http", "telnet", "te30", "te40", "te50", "te60"] },
