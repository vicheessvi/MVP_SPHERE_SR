(function (root, factory) {
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-automatic-polling-plan", title: "План автоматического опроса", summary: "План выбирает одно устройство по точному IP либо строит набор каскадом Домен → Тип оборудования → Производитель → Модель.", details: "В режиме «По IP-адресу» устройство ищется только среди актуальной SR по полному точному IPv4. Карточка под полем показывает данные SR и доступность опроса; похожий, исторический, отсутствующий или неоднозначный адрес не разрешает запуск. В режиме «По фильтрам» каждый уровень допускает один, несколько или все доступные варианты, а пустой домен — «Не указано». «Все» не сочетается с частными значениями. Дата задаёт начало, а Интервал — ожидание после сохранения JSON и до следующего устройства. Неподдерживаемые устройства учитываются, но сетевые запросы к ним не выполняются.", keywords: ["план", "ip-адрес", "одно устройство", "домен", "тип оборудования", "производитель", "модель", "все", "не указано", "интервал"] },
