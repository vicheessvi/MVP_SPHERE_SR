(function (root, factory) {
  ...
  const MODULE_CATALOG = deepFreeze([
    ...
    ...equipmentModules,
    {
      ...
      details: "Операционный экран использует каскад актуальной SR: Тип оборудования, Производитель, Модель. Он показывает выбранные, поддерживаемые и неподдерживаемые устройства. XLSX учётных данных содержит только «Логин» и «Пароль»; секреты остаются в памяти текущего job. После выбора общей папки кнопка формирует план и запускает защищённый loopback runtime; каждый JSON сохраняется до интервала. Для автоматического опроса проект запускается двойным щелчком по START_MVP_SPHERE_SR.cmd; вводить команду PowerShell не требуется. Прямой index.html сохраняет только ручной импорт.",
