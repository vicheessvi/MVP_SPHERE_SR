(function (root, factory) {
  ...
  const HUAWEI_TE_MODELS = deepFreeze(["te30", "te40", "te50", "te60"]);
  ...
  const ANALYZED_PARAMETER_RULES = deepFreeze([
    ...
    ...HUAWEI_TE_MODELS.flatMap((modelNormalized) => [
      { id: `huawei-${modelNormalized}-model-v1`, category: "vcs", manufacturerNormalized: "huawei", modelNormalized, path: "$.webBlocks.Device Info.Model", label: "Модель терминала ВКС", rationale: `Подтверждает модель Huawei ${modelNormalized.toUpperCase()} в результате общего алгоритма опроса.`, version: 1 },
  ...
  const equipmentModules = EQUIPMENT_CATEGORY_CATALOG.map((category) => ({
    ...
    details: `Использует общий перечень, фильтры, карточку, историю опросов и аналитику оборудования. Категория определяется точным сравнением поля SR «${category.srField}» со значением «${category.srValue}» после безопасной нормализации пробелов и регистра. Каждая различимая строка актуальной SR остаётся в перечне даже без результата опроса.${["controller", "panel"].includes(category.id) ? " Для устройств Extron доступен отдельный локальный contract-based web-опрос; ручная загрузка JSON и папок сохраняется." : category.id === "vcs" ? " Для Huawei TE30, TE40, TE50 и TE60 доступен один guarded web-CGI опрос; остальные модели требуют подтверждения собственного протокола. Ручная загрузка JSON и папок сохраняется." : " Подтверждённый протокол автоматического опроса не предполагается без отдельной спецификации."}`,
  ...
  const STATUS_DESCRIPTORS = deepFreeze([
    ...
    { id: "status-auth", group: "pollStatuses", code: "authorization_error", summary: "Для любого устройства Extron получено точное значение error = No credentials were accepted либо для поддерживаемого Extron/Huawei TE30/TE40/TE50/TE60 подтверждён отказ на стадии авторизации.", keywords: ["authorization_error", "no credentials were accepted", "логин", "пароль", "extron", "huawei", "te30", "te40", "te50", "te60"] },
