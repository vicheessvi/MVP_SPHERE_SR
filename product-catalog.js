(function (root, factory) {
  ...
  const ANALYZED_PARAMETER_RULES = deepFreeze([
    ...
    { id: "huawei-te40-model-v1", category: "vcs", manufacturerNormalized: "huawei", modelNormalized: "te40", path: "$.webBlocks.Device Info.Model", label: "Модель терминала ВКС", rationale: "Подтверждает применение отдельного алгоритма опроса Huawei TE40.", version: 1 },
    { id: "huawei-te40-firmware-v1", category: "vcs", manufacturerNormalized: "huawei", modelNormalized: "te40", path: "$.webBlocks.Firmware.Version", label: "Версия прошивки", rationale: "Показывает установленную версию прошивки терминала ВКС Huawei TE40.", version: 1 }
  ]);
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-huawei-te40-web-polling", title: "Автоматический опрос Huawei TE40", summary: "Для терминала ВКС Huawei TE40 применяется отдельный подтверждённый алгоритм входа и чтения параметров web-интерфейса.", details: "Алгоритм выбирается только при точном сочетании категории «Терминалы ВКС», производителя Huawei и модели TE40. Он использует HTTPS по выбранному IP, воспроизводит подтверждённую последовательность браузерной авторизации и читает только разрешённый набор сведений об устройстве, прошивке, локальном времени, сети и возможностях. Учётные данные, cookie и CSRF-токен существуют только в памяти задания и не записываются в итоговый JSON. Самоподписанный сертификат разрешается явно только для текущего запуска.", keywords: ["huawei", "te40", "терминал вкс", "автоматический опрос", "https", "mac", "прошивка"] },
  ...
  const equipmentModules = EQUIPMENT_CATEGORY_CATALOG.map((category) => ({
    ...
    details: `Использует общий перечень, фильтры, карточку, историю опросов и аналитику оборудования. Категория определяется точным сравнением поля SR «${category.srField}» со значением «${category.srValue}» после безопасной нормализации пробелов и регистра. Каждая различимая строка актуальной SR остаётся в перечне даже без результата опроса.${["controller", "panel"].includes(category.id) ? " Для устройств Extron доступен отдельный локальный contract-based web-опрос; ручная загрузка JSON и папок сохраняется." : category.id === "vcs" ? " Для Huawei TE40 доступен отдельный локальный web-опрос; остальные модели требуют подтверждения собственного протокола. Ручная загрузка JSON и папок сохраняется." : " Подтверждённый протокол автоматического опроса не предполагается без отдельной спецификации."}`,
  ...
  const STATUS_DESCRIPTORS = deepFreeze([
    ...
    { id: "status-auth", group: "pollStatuses", code: "authorization_error", summary: "Для любого устройства Extron получено точное значение error = No credentials were accepted либо для поддерживаемого Extron/Huawei TE40 подтверждён отказ на стадии авторизации.", keywords: ["authorization_error", "no credentials were accepted", "логин", "пароль", "extron", "huawei", "te40"] },
