(function (root, factory) {
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-poll-results-storage", title: "Общая папка результатов опросов", summary: "Локальный исполнитель создаёт в выбранном корне папку запуска YYYY-MM-DD_HH-mm-ss и отдельный JSON для каждого результата.", details: "JSON сохраняется атомарно сразу после завершения устройства. После успешной записи начинается заданный интервал перед следующим поддерживаемым устройством. Эту же общую папку можно вручную импортировать в «Загрузке».", keywords: ["poll-results", "папка результатов", "json", "интервал"] },
