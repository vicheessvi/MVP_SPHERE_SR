(function (root, factory) {
  ...
  const POLLING_HELP_ENTRIES = deepFreeze([
    ...
    { id: "logic-sr-xlsx", title: "Выгрузка SR", summary: "XLSX с актуальным перечнем оборудования формирует варианты Домена, Типа оборудования, Производителя и Модели.", details: "Читается первый непустой лист. Обязательные столбцы проверяются до изменения сеансового состояния. Необязательный столбец «Домен» используется как первый уровень фильтра Плана автоматического опроса; пустое значение доступно как «Не указано». Диагностические предупреждения не означают частичную обработку, если ни одна строка не отклонена.", keywords: ["sr", "xlsx", "первый лист", "импорт", "домен"] },
